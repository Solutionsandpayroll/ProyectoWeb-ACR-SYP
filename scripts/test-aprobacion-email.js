/**
 * Script temporal para probar el envío de correo de aprobación de ACR
 * sin crear un ACR real en la base de datos.
 * 
 * Uso: node scripts/test-aprobacion-email.js
 */
...
// Configuración SMTP (copiada de lib/email.ts)
...
const EMAIL_FROM = `${FROM_NAME} <${FROM_ADDRESS}>`;
...
// Plantilla HTML (copiada de lib/email-templates/acr-aprobacion-pendiente.ts)
function buildAcrAprobacionPendienteHtml(data) {
  const { consecutivo, proceso, cliente, registrado_por, autorizado_por, acr_url } = data;
...
// Datos de prueba (usando ACR real)
const testData = {
  consecutivo: '018',
  proceso: 'Employer of Record',
  cliente: 'REMOFIRST',
  registrado_por: 'German Hincapie',
  autorizado_por: 'William Romero',
  acr_url: 'https://acr.solutionsandpayroll.com/dashboard/historial-acr/27',
};

// Enviar correo de prueba
async function sendTestEmail() {
  console.log('=== PRUEBA DE CORREO DE APROBACIÓN DE ACR ===\n');
  console.log('Enviando correo a: automatizacion2@solutionsandpayroll.com');
  console.log('Datos de prueba:', testData);
  console.log('');

  try {
    const htmlContent = buildAcrAprobacionPendienteHtml(testData);
    
    const info = await transporter.sendMail({
      from: EMAIL_FROM,
      to: 'automatizacion2@solutionsandpayroll.com',
      subject: `Nueva ACR pendiente de aprobación - ${testData.consecutivo}`,
      html: htmlContent,
    });

    console.log('✓ Correo enviado exitosamente');
    console.log('Message ID:', info.messageId);
    console.log('\nPuedes revisar tu bandeja de entrada en: automatizacion2@solutionsandpayroll.com');
  } catch (error) {
