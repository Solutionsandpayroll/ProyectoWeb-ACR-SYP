...
export default function GdsControlCambiosPage() {
  ...
  async function fetchData() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/gds-control-cambios");
  ...
  return (
    <div className="flex flex-col flex-1 min-h-screen bg-slate-50">
      <Header title="Control de Cambios GDC" />

      <main className="flex-1 px-4 sm:px-6 py-6 sm:py-8 max-w-4xl mx-auto w-full">
        {/* Page header */}
        <div className="flex items-center justify-between gap-3 flex-wrap mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Control de Cambios del Formato GDC</h1>
            <p className="text-sm text-slate-500 mt-1">
              Historial de versiones y modificaciones al formato de Gestión del Cambio (GIN).
            </p>
            {!isAdmin && session && (
        ...
        {showForm && isAdmin && (
          ...
          >
            <h2 className="text-sm font-bold uppercase tracking-widest text-[#105789]">
              Nuevo registro de cambio
            </h2>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Describe brevemente qué se modificó en el formato GDC…"
                value={descripcion}
        ...
        {loading ? (
          ...
          <div className="text-center py-24 text-slate-400">
            ...
            <p className="text-xs mt-1">
              {isAdmin
                ? 'Haz clic en "Registrar cambio" para agregar el primero.'
                : "Cuando exista historial, lo verás aquí en modo lectura."}
            </p>
          </div>
        ) : (
            ...
            <div className="space-y-0">
              {cambios.map((c, idx) => (
                <div key={c.id} className="relative flex flex-col md:flex-row gap-3 md:gap-6 pb-8 last:pb-0">
                  ...
                  {/* Card */}
                  <div className="flex-1 bg-white border border-slate-200 rounded-xl shadow-sm px-5 py-4 space-y-2">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="space-y-0.5">
                        <p className="text-xs text-slate-500">{fmtDate(c.fecha)}</p>
                        {c.autor && (
                          <p className="text-xs font-medium text-slate-600">por {c.autor}</p>
                        )}
                      </div>
                      {isAdmin && (
                        <div>
                          {deletingId === c.id ? (
                            <div className="flex gap-2 text-xs">
                              <button
                                ...
                                className="text-red-600 hover:text-red-800 font-semibold px-2 py-1 rounded border border-red-200 bg-red-50 transition"
                              >
                                Confirmar
                              </button>
                              <button
                                onClick={() => setDeletingId(null)}
                                className="text-slate-500 hover:text-slate-700 font-medium px-2 py-1 rounded border border-slate-200 hover:bg-slate-50 transition"
                              >
                                Cancelar
                              </button>
                            </div>
